...
{
public:
    Flv2TsStreamerMgr(const char* output_filename):filename_(output_filename) {
    }
...
public:
    int MakeStreamers() {
        ...
        muxer_ = CppStreamerFactory::MakeStreamer("mpegtsmux");
        if (!muxer_) {
            LogErrorf(logger_, "make streamer tsmux error");
            return -1;
        }
        LogInfof(logger_, "make mpegts mux streamer:%p, name:%s", muxer_, muxer_->StreamerName());
...
int main(int argc, char** argv) {
    ...
    auto streamer_mgr_ptr = std::make_shared<Flv2TsStreamerMgr>(output_ts_name);
